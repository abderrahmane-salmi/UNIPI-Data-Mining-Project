...
class ArtistImputer:
    ...
    def _impute_row(self, row: pd.Series) -> pd.Series:
        ...
        if not entity:
            print("peso")
            self._log_imputation(row.name, title, {}, None)
            return row
        ...
        self._log_imputation(row.name, title, applied, region_info)
        return row
    ...
    def _resolve_region(
        ...
        wiki_region = self._infer_location_from_wikipedia(title)
        if wiki_region:
    ...
    def _infer_location_from_wikipedia(self, title: str) -> Optional[str]:
        ...
        for name, _ in ranked:
            normalized = self._normalize_location_label(name)
            if normalized:
                return normalized
        return ranked[0][0]
